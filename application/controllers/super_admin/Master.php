...
class Master extends CI_Controller
{
	public function __construct()
	{
		...
		$this->load->model('super_admin/master_model', 'master');
	...
	public function village($dt = NULL)
	{
		if ($dt == 'viewList') {
			...
			foreach ($record as $row) {
				...
				$viewClick = "'" . $row->id . "','super_admin/master/village/viewDetails','view'";
				$editClick = "'" . $row->id . "','super_admin/master/village/viewDetails','edit'";
			...
			if ($this->form_validation->run() == TRUE) {
				...
				$updtArr = array(
					'district_id' => $post['district'], 'name' => $post['village_name'], 'village_category' => $post['vill_cate'], 'distance' => $post['distance'], 'open_date' => $post['openingDate'], 'no_center' => $post['noCentr'],
					'modified_id' => $this->logId, 'staff_id' => $post['staffID'], 'modified_date' => config_item('work_end')
				);
				...
				$msg = array(
					'district' => form_error('district'), 'vill_cate' => form_error('vill_cate'), 'village_name' => form_error('village_name'),
					'distance' => form_error('distance'), 'openingDate' => form_error('openingDate'), 'staffID' => form_error('staffID'),
			...
			if ($this->form_validation->run() == TRUE) {
				$post = $this->input->post();
				$createArr = array(
					'district_id' => $post['district'], 'vlg_code' => $post['vCode'], 'name' => $post['village_name'], 'village_category' => $post['vill_cate'],
					'distance' => $post['distance'], 'open_date' => $post['openingDate'], 'no_center' => $post['noCentr'],
					'created_id' => $this->logId, 'staff_id' => $post['staffID'], 'create_date' => config_item('work_end')
				);
			...
			$data['getEmployee'] = $this->common->all_data_con('staff', array('status' => '1'), 'id,staff_id,full_name');
			$data['target']    = 'super_admin/master/village/viewList';
	...
	public function center($dt = NULL)
	{
		...
		if ($dt == 'viewList') {
			...
			foreach ($record as $row) {
				...
				$getUid = urlencode(base64_encode($row->id));
				$getUsername = urlencode(base64_encode($row->name));
				$viewClick = "'" . $row->id . "','super_admin/master/center/viewDetails','view'";
				$editClick = "'" . $row->id . "','super_admin/master/center/viewDetails','edit'";
				...
				$return['data'][] = array(
					'<strong>' . $i++ . '.</strong>', '<strong>' . $row->center_id . '</strong>', $row->center_name,
			...
			if ($this->form_validation->run() == TRUE) {
				$conArr = array('id' => $post['target']);
				$updtArr = array(
					'vll_id' => $post['villageID'], 'center_name' => $post['center_name'], 'open_date' => $post['openingDate'],
					'staff_id' => $post['staffID'], 'distance' => $post['distance'], 'no_of_grp' => $post['noGroup'],
					'modified_id' => $this->logId, 'modified_date' => config_item('work_end')
				);
				...
				$msg = array(
					'villageID' => form_error('villageID'), 'center_name' => form_error('center_name'), 'openingDate' => form_error('openingDate'),
					'staffID' => form_error('staffID'), 'distance' => form_error('distance'), 'noGroup' => form_error('noGroup')
				);
				$data = array('icon' => 'error', 'text' => $msg);
			}
			echo json_encode($data);
			...
			$data['getVillage'] = $this->common->all_data_con('master_village', array('status' => '1'), 'id,name,vlg_code');
			$data['getEmployee'] = $this->common->all_data_con('staff', array('status' => '1'), 'id,staff_id,full_name');
			$data['target']    = 'super_admin/master/center/viewList';
			$this->load->view('super_admin/base', $data);
		}
	}
	public function branch($dt = NULL)
	{
		if ($dt == 'viewList') {
			...
			foreach ($record as $row) {
				...
				$getUid = urlencode(base64_encode($row->id));
				$getUsername = urlencode(base64_encode($row->name));
				$viewClick = "'" . $row->id . "','super_admin/master/branch/viewDetails','view'";
				$editClick = "'" . $row->id . "','super_admin/master/branch/viewDetails','edit'";
				$loginMember = "'" . $row->id . "','super_admin/master/branch/loginMember','loginMem'";
				...
				$actionBtn .= ' <a href="javascript:void(0)" onclick="manageBranch(' . $loginMember . ')" class="btn btn-outline-dark btn-sm waves-effect btn-padd" title="partner login Radhe"><i class="bx bx-log-in-circle"></i> </a>';
				$return['data'][] = array(
					'<strong>' . $i++ . '.</strong>', '<strong>' . $row->br_id . '.</strong>', $row->branch_name,
			...
			$return['draw'] = $post_data['draw'];
			echo json_encode($return);
		    } else if ($dt == 'loginMember') { 

		    $post_id = $this->input->post();

			$bra_det = $this->common->get_data('master_branch',array('id' => $post_id['id']),'id,branch_name,email_id,address,status');

			$user_det = $this->common->get_data('users',array('id' => '390'),'id,email,password,show_ps,department_type,status,name');

			$datata = array(
				'branch_id'       =>    $bra_det['id'],
				'branch_office'   =>    $bra_det['branch_name'],
				'user_id'         =>    $user_det['id'],
				'name'            =>    $user_det['name'],  
				'staff_id'        =>   $user_det['staff_id'],
				'email'           =>    $user_det['email'],
				'password'        =>    $user_det['password'],
				'user_cate'       =>    $user_det['department_type'],
			);

			$data = $this->session->set_userdata($datata);
			$data = array('text' => 'Login Successfully', 'icon' => 'success');
			echo json_encode($data);
			...
			if ($this->form_validation->run() == TRUE) {
				$post = $this->input->post();
				$createArr = array(
					'br_id' => $post['branchCode'], 'branch_name' => $post['branch_name'],
					...
					'created_id' => $this->logId, 'create_date' => config_item('work_end')
				);

					if ($this->common->save_data('master_branch', $createArr)) {
					$data = array('icon' => 'success', 'text' => 'Thank You! you have successfully create');
					} else {
					$data = array('icon' => 'error', 'text' => array('Oops it seems somthing went wrong please reload site.'));
				    }
				...
				$msg = array(
					'branch_name' => form_error('branch_name'), 'mobile' => form_error('mobile'), 'email' => form_error('email'), 'off_addr' => form_error('off_addr'),
					'state' => form_error('state'), // 'district' => form_error('district'), 
					'zipcode' => form_error('zipcode'), 'openingDate' => form_error('openingDate')
				);
				$data = array('icon' => 'error', 'text' => $msg);
			}
			echo json_encode($data);
			...
			if ($this->form_validation->run() == TRUE) {
				...
				$updtArr = array(
					...
					'opening_date' => $post['openingDate'],
					'modified_by' => $this->logId, 'modified_date' => config_item('work_end')
				);
				...
				$msg = array(
					'branch_name' => form_error('branch_name'), 'mobile' => form_error('mobile'), 'email' => form_error('email'), 'off_addr' => form_error('off_addr'),
					'state' => form_error('state'), 'district' => form_error('district'), 'zipcode' => form_error('zipcode'), 'openingDate' => form_error('openingDate')
				);
				$data = array('icon' => 'error', 'text' => $msg);
			}
			echo json_encode($data);
			...
			$data['target']    = 'super_admin/master/branch/viewList';
			$this->load->view('super_admin/base', $data);
		}
	}
	...
	public function group($dt = NULL)
	{
		if ($dt == 'viewList') {
			...
			foreach ($record as $row) {
				...
				$getUid = urlencode(base64_encode($row->id));
				$getUsername = urlencode(base64_encode($row->name));
				$viewClick = "'" . $row->id . "','super_admin/master/group/viewDetails','view'";
				$editClick = "'" . $row->id . "','super_admin/master/group/viewDetails','edit'";
				...
				$return['data'][] = array(
					...
					$row->name,
					'<span style="text-align:center;">'.$row->center_name.'</span>',
			...
			if ($this->form_validation->run() == TRUE) {
				$conArr = array('id' => $post['target']);
				$updtArr = array(
					'name' => $post['group_name'], 'create_date' => $post['openingDate'],
					'staff_id' => $post['staffID'], 'center_id' => $post['centerID'],
					'modified_by' => $this->logId, 'modified_date' => config_item('work_end')
				);
			...
			$arrGroup = array(
				'id' => $getGroup['id'], 'grp_id' => $getGroup['grp_id'], 'center_id' => $getGroup['center_id'], 'staff_id' => $getGroup['staff_id'], 'name' => $getGroup['name'],
			...
			$data['getCenter'] = $this->common->all_data_con('master_center', array('status' => '1'), 'id,center_name,center_id');
			$data['getEmployee'] = $this->common->all_data_con('staff', array('status' => '1'), 'id,staff_id,full_name');
			$data['target']    = 'super_admin/master/group/viewList';
			$this->load->view('super_admin/base', $data);
		}
	}
	...
	public function field_schedule($dt = NULL)
	{
		if ($dt == 'viewList') {
			...
			foreach ($record as $row) {
				...
				$getUid = urlencode(base64_encode($row->id));
				$getUsername = urlencode(base64_encode($row->name));
				$viewClick = "'" . $row->id . "','super_admin/master/field_schedule/viewDetails','view'";
				$editClick = "'" . $row->id . "','super_admin/master/field_schedule/viewDetails','edit'";
				$actionBtn = '<a href="javascript:void(0)" onclick="manageFieldSchedule(' . $viewClick . ')" class="btn btn-outline-primary btn-sm waves-effect btn-padd" title="View"><i class="mdi mdi-eye"></i></a>
					<a href="javascript:void(0)" onclick="manageFieldSchedule(' . $editClick . ')" class="btn btn-outline-warning btn-sm waves-effect btn-padd" title="Edit"><i class="mdi mdi-square-edit-outline me-1"></i></a>';
			...
			$data['getMsCenter'] = $this->common->all_data_con('master_center', array('status' => '1'), 'id,center_id,center_name');
			$data['getEmployee'] = $this->common->all_data_con('staff', array('status' => '1'), 'id,staff_id,full_name');
			$data['target'] = 'super_admin/master/field_schedule/viewList';
			$this->load->view('super_admin/base', $data);
		}
	}
}
