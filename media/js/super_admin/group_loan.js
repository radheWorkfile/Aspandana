...
$("#view_chart_btn_fixed").on('click', function() {
    ...
    var amt = $('#amount').val();
    var work_end_date = $('#work_end').val();
    ...
    for (i = 1; i <= tenure; i++) {
        ...
        if (i > 1) {
            ...
            if (tenure_type == 1) {
                var repeat_w_date = 7 * i - 7;
                var last = new Date(next_emi_date);
                last.setDate(last.getDate() + repeat_w_date);
            } else if (tenure_type == 2) {
                var repeat_bw_date = 14 * i - 14;
                var last = new Date(next_emi_date);
                last.setDate(last.getDate() + repeat_bw_date);
            }
